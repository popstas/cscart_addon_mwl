...
{capture name="tabsbox"}
    ...
    <div id="content_general">
        {hook name="profiles:general_content"}
            ...
            {if !$id}
            {* mwl_xlsx: usergroup select and send password link checkbox on add *}
            <div class="control-group">
                <label for="elm_usergroup_id" class="control-label">{__("usergroup")}:</label>
                <div class="controls">
                    <select name="user_data[usergroup_id]" id="elm_usergroup_id">
                        <option value="0">{__("not_a_member")}</option>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="checkbox" name="send_password_setup_link" id="send_password_setup_link" value="Y" />
                    <label for="send_password_setup_link">{__("mwl_xlsx.send_recover_links")}</label>
                </div>
            </div>
            {/if}
                ...
                {foreach from=$usergroups item=usergroup}
                    <tr>
                        ...
                        <td class="right" data-th="{__("status")}">
                            ...
                            {/if}
                            {include file="common/select_popup.tpl" type="usergroups" id=$usergroup.usergroup_id status=$ug_status hidden="" items_status="profiles"|fn_get_predefined_statuses extra="&user_id=`$id`" update_controller="usergroups" notify=true notify_text=__("notify_user") hide_for_vendor=$hide_for_vendor}
                        </td>
                    </tr>
                {/foreach}
                </table>
...
<div class="btn-group btn-hover dropleft">
    {hook name="profiles:update_buttons"}
        ...
        <ul class="dropdown-menu">
            {* mwl_xlsx override: notify_customer unchecked by default for user_type C *}
            <!-- template: mwl_xlsx overrides/views/profiles/update.tpl -->
            <li data-mwl-xlsx-override="profiles/update"><a><input type="checkbox" name="notify_customer" value="Y" {if ($smarty.request.user_type|default:$user_data.user_type) != 'C'}checked="checked" {/if} id="notify_customer" />
                {__("notify_user")}</a></li>
        </ul>
    {/hook}
</div>
...
{if !$id}
{* mwl_xlsx: random password generator for new customer profiles *}
<script>
(function() {
    var password1 = document.getElementById('password1');
    var password2 = document.getElementById('password2');
    if (!password1 || !password2) return;
    var controls = password1.closest('.controls');
    if (!controls) return;
...
{/if}
{* mwl_xlsx: open Create/Save dropdown on hover *}
<script>
(function() {
    var group = document.querySelector('.btn-group.btn-hover.dropleft');
    if (!group) return;
    ...
    group.addEventListener('mouseleave', function() {
        closeTimer = setTimeout(function() { group.classList.remove('open'); }, 200);
    });
    if (menu) {
