            ...
            <ul class="ty-range-slider__wrapper">
                <li class="ty-range-slider__item" style="left: 0%;">
                    <span class="ty-range-slider__num">
                        <span><bdi>{$filter.prefix nofilter}<span>{if $addons.mwl_xlsx.compact_price_slider_labels != "Y"}{$min|shortnum}{else}{$min}{/if}</span>{$filter.suffix nofilter}</bdi></span>
                    </span>
                </li>
                <li class="ty-range-slider__item" style="left: 100%;">
                    <span class="ty-range-slider__num">
                        <span><bdi>{$filter.prefix nofilter}<span>{if $addons.mwl_xlsx.compact_price_slider_labels != "Y"}{$max|shortnum}{else}{$max}{/if}</span>{$filter.suffix nofilter}</bdi></span>
                    </span>
                </li>
            </ul>
        </div>
